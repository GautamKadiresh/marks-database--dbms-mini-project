...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($countBatch==1){
        $delBatch=mysqli_multi_query($link,"DELETE FROM batch WHERE batch_code='$bcode';DELETE FROM student WHERE batch_code='$bcode';DELETE FROM marks WHERE batch='$bcode';");
        echo "<script>
        alert('Batch Deleted!');
